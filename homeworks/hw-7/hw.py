...
sql_str = """
    SELECT * INTO movies_top FROM (
        SELECT movieid, count(userid) as ratings_num, avg(rating) as ratings_avg FROM ratings
        GROUP BY movieid
    ) as movies
    """
...
engine = create_engine(
    'postgresql://postgres:@{}:{}'.format(os.environ['APP_POSTGRES_HOST'], os.environ['APP_POSTGRES_PORT']))
